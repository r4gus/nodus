...
impl Plugin for UndoPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_event::<UndoEvent>()
            .insert_resource(UndoStack { undo: Vec::new(), redo: Vec::new() })
                    ...
                    match action {
                        Action::Insert(e) => {
                            ...
                            match e.ntype {
                                ...
                                NodeType::HighConst => {
                                    let id = Gate::high_const(&mut commands, e.position, font.clone());
                                },
                                NodeType::LowConst => {
                                    let id = Gate::low_const(&mut commands, e.position, font.clone());
                                },
...
pub fn detect_removal_system(
    mut stack: ResMut<UndoStack>,
    removals: RemovedComponents<NodeType>,
    q_node: Query<(
        Entity, 
        &Name, 
        Option<&Inputs>, 
